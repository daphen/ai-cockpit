...
check "headless editor has an explicit owner" "$(grep -c 'nvim.*--headless.*--listen' TermView.cpp)" "1"
check "terminal attaches as a remote UI" "$(grep -c 'nvim --server.*--remote-ui' TermView.cpp)" "1"
